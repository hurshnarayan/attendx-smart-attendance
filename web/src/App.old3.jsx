...
/* ---------------- TeacherView (unchanged behaviour, minor cache-bust) ---------------- */
...
/* Approve/Reject buttons use the same endpoints as before (class/session is resolved by dashboard) */
...
/* ---------------- DashboardPanel (patched robust version) ---------------- */
function DashboardPanel() {
  ...
  useEffect(() => {
    ...
    function onStorageChange() {
      // always re-read localStorage so multi-tab scenarios work
  ...
  // Clear by using component state for scope (robust) and suppress refetch for a short window after clearing.
  async function clearData(
    ...
    try {
      ...
      // remove any stray local cache keys
      try {
      ...
      // success: optimistic clear and suppress refetch for a short window to avoid race
      setPresent([]);
      setPending([]);
      setFlagged([]);
