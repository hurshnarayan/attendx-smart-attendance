  ...
  async function exportAndClear() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `attendance_backup_${new Date()
          .toISOString()
          .slice(0, 10)}.csv`;
        document.body.appendChild(a);
        a.click();
      ...
      <Section title={`Flagged (${flagged.length})`}>
        {flagged.length === 0 ? (
          ...
          flagged.slice(0, 6).map((s) => (
            <div
              key={s.id}
              className="py-2 border-b border-gray-100 last:border-0 bg-amber-50 -mx-2 px-2 rounded"
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="font-medium text-gray-900">
